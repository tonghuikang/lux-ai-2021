...
class Player:
    ...
    def researched_coal_projected(self) -> bool:
        return self.research_points + self.city_tile_count * 2 >= GAME_CONSTANTS["PARAMETERS"]["RESEARCH_REQUIREMENTS"]["COAL"]

    def researched_uranium_projected(self) -> bool:
        return self.research_points + self.city_tile_count * 2 >= GAME_CONSTANTS["PARAMETERS"]["RESEARCH_REQUIREMENTS"]["URANIUM"]
...
class City:
    def __init__(self, teamid, cityid, fuel, light_upkeep, night_turns_left):
        ...
        self.fuel_needed_for_night = max(500, light_upkeep * min(night_turns_left%10, 10) - fuel - self.fuel_on_the_way)
